...
export default function Cart() {
  let 장바구니 = ["Tomatoes", "Pasta", "orange"];
  let card = ["현대", "롯데", "신한"];
  return (
    <div>
      <Contents />
      ...
      <CartItem item={장바구니[1]} temp={card[1]} />
      <CartItem item={장바구니[2]} temp={card[2]} />
      <Banner event={card[0]} />
      <Banner event={card[1]} />
      <Banner event={card[2]} />
      <RedBtn color="blue" />
      <Link style={{ textAlign: "center" }} href="/cart/payment">
        결제
      </Link>
    </div>
  );
...
function RedBtn(props) {
  return (
    <button
      style={{ background: props.color, width: "100px", marginLeft: "400px" }}
    >
      버튼
    </button>
  );
